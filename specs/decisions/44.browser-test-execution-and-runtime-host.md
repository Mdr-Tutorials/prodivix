...
- 日期：2026-07-15
- ImplementationStatus：Browser Test Vertical Slice + Shared Runtime Host Implemented
...
flowchart LR
    ...
    Compiler --> Snapshot["BrowserProjectSnapshot"]
...
当前 Browser adapter 实现 Vitest 纵切：

1. Host 同步 exact snapshot 并按需安装依赖。
2. Test provider 运行 snapshot 中声明的 Vitest command。
3. Adapter 从保留路径读取 Vitest JSON，验证并转换为 canonical `ExecutionTestReport`。
...
- URI：`browser-project://{workspaceId}/{snapshotId}/{reportFilePath}`。
...
Browser adapter 只拥有 Vitest JSON decoder。未来 Remote provider 或其他测试工具在 adapter 边界转换为同一 report contract；Web 不解析工具私有 JSON，也不依赖容器供应商 SDK。
...
- Browser project test plan 与 Vitest report decoder；
...
- [x] Vitest JSON 在 Browser adapter 边界转换为 transport-neutral `ExecutionTestReport`。
...
- [ ] Remote Test provider 通过同一 ExecutionProvider 与 report conformance。
