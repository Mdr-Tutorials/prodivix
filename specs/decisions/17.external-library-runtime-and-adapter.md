...
采用“**Plugin Contribution + Canonical External IR + 通用 Runtime 内核 + 可选 Policy 增强**”架构：
...
0. **Plugin Contribution（外部能力归属层）**
   ...
   - 当前 core-embedded profile 的 Palette projection 已进入 Plugin Host registry；库专属 runtime、manifest、render/codegen policy 仍是 Browser Sandbox 与 official plugin 迁移前的过渡实现，不是长期插件 API。
   - Phase 4.5 已冻结 `externalLibrary@1.0`、`renderPolicy@1.0`、`codegenPolicy@1.0` 与 `iconProvider@1.0`，并完成 Host-controlled package resolution、owner/generation implementation binding、canvas projection 与 compiler snapshot；Ant Design/MUI/Radix 的实际迁移仍按后续 phase 直接替换。
...
## 接入分层模型（Plugin Manifest 强制，Library Enhancement Manifest 可选）

这里的“可选 Manifest”仅指某个外部库的 props、分组和 codegen hints 增强描述，不是 Plugin Manifest。任何可安装 official / community plugin 都必须通过 `plugin-manifest-v1.schema.json` 声明身份、权限和 contribution。
...
### L2：声明式 Library Enhancement Manifest（可选）
...
`Extension Host -> externalLibrary contribution -> LibraryDescriptor -> RuntimeLoader -> ExportScanner -> Canonical IR -> resolved contribution registry -> Palette/Tree/Inspector -> CanvasRenderer -> CodeGenerator`
...
1. 通过官方 generic external-library plugin 支持用户注册任意 esm.sh 库，并把项目状态持久化到 Workspace VFS。
2. 将常见库迁移为使用同一宿主协议的 official plugin，并提供社区插件模板。
