...
- Remote Terminal transport 与 durable Job event 分离：Web bearer 只保存在 ref，resume 旋转 access token；
  Control Plane 只保存 digest，stdin 仅在有界未确认 mailbox 中存在；ADR 50 进一步把 Core/redactor checkpoint、
  mailbox、digest 与 cursor 作为 AES-256-GCM sealed state 持久化到 PostgreSQL，并以 exact identity/revision CAS
  支持跨 Control Plane 副本接续，数据库不接触 plaintext。Worker 只在 runtime 断网后打开 inner
...
- artifact expiry、quota、authorization、network denial 与跨 region database/worker/traffic failover 的完整 recovery parity；
- 有界终端仿真器已由 ADR 53 完成 first vertical；Structured Console/Files 的其余 SourceTrace/Issues/Data 导航与
  Golden 调试旅程继续建设；
- 第二 managed KMS provider、CloudHSM 与完整跨表面 Secret Gate。Backend Environment Secret 和 Remote Terminal 的 AWS managed KMS first vertical 已关闭；Remote Terminal 已覆盖 transport-wide/cross-chunk、
...
- [x] Remote Terminal Core/redactor checkpoint、AES-GCM key ring、PostgreSQL opaque state/revision CAS 与双副本
      client/worker continuation 已形成 Gate；lease 续期与 worker generation drift 通过同一 sweep/CAS fence。
- [x] Remote Terminal PRT2 使用 per-revision data key、AWS KMS exact ARN/hashed context、PRT1 decrypt-only migration、
      retryable outage revision fence 与 related MRK regional broker continuation；live OIDC/MRK 证据由独立 Gate 获取。
- [x] Regional Remote recovery 使用 repeatable-read exact checkpoint、shared request drain/exclusive traffic epoch、
      durable cutover evidence、双 Control Plane standby/readiness hard cut、same-worker continuation、attempt+1 reclaim
      与 Terminal generation replacement；真实云端 database/traffic RPO/RTO evidence 由独立 Gate 获取。
