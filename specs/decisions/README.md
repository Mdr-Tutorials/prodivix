...
| `40.execution-provider-and-job.md`                          | Accepted    | Contract + Browser Providers + Remote Control Plane/Worker D2 + Rootless Sandbox Implemented | Revision-bound ExecutionRequest/Job/Session、Browser Preview/Test、neutral snapshot、Remote Control Plane/PostgreSQL/HTTP、Worker Agent、durable event/artifact blob/budget/retention，以及 rootless Podman sandbox/GitHub Gate 已落地。                                                  | 生成远端 Isolation Gate 证据，并实现 Remote provider 与 resolver/permission。                        |
...
| `44.browser-test-execution-and-runtime-host.md`             | Accepted    | Browser Test + Shared Runtime Host + Neutral Snapshot Implemented                            | Preview/Test 使用独立 provider descriptor、Job 与 Session，共享 Browser Runtime Host 的 filesystem/dependency lifecycle；Compiler 生产 neutral snapshot，Vitest 结果转换为 runtime-core `ExecutionTestReport`。                                                                           | 完成 artifact resolver、Remote Test、Data mock/live safety 与两 target parity；Evidence 留在 G3。    |
