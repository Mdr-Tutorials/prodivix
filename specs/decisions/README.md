...
| `31.production-export-planner.md`                           | Draft       | G1 Planner + Standalone Build/Browser Gate Implemented / G2 Multi-target Planned      | `ExportProgram`、`ProductionExportPlanner`、package-manager/build policy、TSX/JSX 输出方言，以及独立项目 install/typecheck/test/build/browser-smoke Gate 已落地。                                                                                                                         | 在 G2 完成多 framework target、source map、tree shaking、code splitting 和 License UI。             |
...
| `40.execution-provider-and-job.md`                          | Accepted    | Contract + Browser Preview/Test Provider Slices Implemented                           | Revision-bound ExecutionRequest、capability matching、instance-owned registry、ExecutionJob lifecycle、Session/test report 与 reference-only environment/Secret identity 已落地；environment request 自动要求 `environment-binding` capability。                                          | 实现 resolver/permission 与 Remote Isolated provider，并让新执行面复用 session/job contract。       |
| `41.project-runner-and-canvas-modes.md`                     | Accepted    | Browser Preview + Shared Host Vertical Slice Implemented                              | 蓝图 Design/Interactive/Run 三模式、独立 React/Vite runtime、HMR、原位 iframe 与共享 Execution Center 已贯通；Preview/Test 以独立 descriptor 共享 Runtime Host，NodeGraph/Animation 复用同一 Session/Console。                                                                            | 扩展 Terminal/Network 与多 framework runner。                                                       |
| `42.nodegraph-execution-session.md`                         | Accepted    | Same-context Vertical Slice Implemented                                               | Domain-owned provider、revision-bound document、实时 trace/log/diagnostic/SourceTrace、Run/Stop、Blueprint trigger、state patch result 与共享 Session/Console 已贯通；旧 browser 直调协议已删除。                                                                                         | 在 G3 扩展 typed data/async/error/CodeSlot executor 与 Behavior Verification。                      |
| `43.animation-runtime-and-execution-session.md`             | Accepted    | Same-context Browser Vertical Slice Implemented                                       | Domain-owned scheduler/effect lease port 与 provider、完整单 timeline lifecycle、generation-fenced Browser effect store、revision-bound Play/Stop/Restart 和共享 Session/Console 已贯通；旧私有 RAF lifecycle 已删除。                                                                    | 在 G3 扩展 composition、route/reduced-motion、CodeSlot/shader 与 VerificationEvidence。             |
| `44.browser-test-execution-and-runtime-host.md`             | Accepted    | Browser Test Vertical Slice + Shared Runtime Host Implemented                         | Preview/Test 使用独立 provider descriptor、Job 与 Session，共享 Browser Runtime Host 的 filesystem/dependency lifecycle；Vitest 结果转换为 runtime-core `ExecutionTestReport`，Test 页面与共享 Execution Center 已贯通。                                                                  | 实现 Remote Test provider，并在 G3 将报告接入正式 scenario/evidence lifecycle。                     |
| `45.data-operation-and-environment-reference-foundation.md` | Accepted    | Contract/Codec + Canonical Workspace/Semantic Foundation Implemented                  | `@prodivix/data` 已定义 current DataSourceDocument、DataOperationReference、schema/policy/lifecycle contract 与 wire codec；`data-source` Workspace document、Semantic Contribution，以及 reference-only environment/Secret identity 已建立。                                             | 接入 PIR/Collection binding、runtime adapter、Secret resolution，并完成 Preview/Export CRUD 纵切。  |
