...
| `29.plugin-extension-points.md`                     | Draft                   | Partial（Contracts / Host / Palette / Phase 4.0） | Manifest/Palette contracts、Host Core、Blueprint resolved Palette、规范 point id、verified runtime artifact 与 Host shutdown 已落地；真实 Browser sandbox/Gateway 尚未实现。                                                                          | 从 Phase 4.1 开始实现 protocol 与 Browser Sandbox，再依次迁移 Ant Design、MUI，并在 compound/portal contract 稳定后迁移 Radix。 |
