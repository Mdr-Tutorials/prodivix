...
| `08.route-manifest-outlet.md`                       | In Progress             | Partial                             | Workspace RouteManifest、page/layout/outlet 引用与后端 `routeRev` 保存链路已存在，但 Route Resolver、`PdxRoute` 收敛与导出链路尚未统一。     | 按 `route-system-unification-plan.md` 推进 RouteGraph 与组件路由统一。                                           |
| `09.component-route-composition.md`                 | In Progress             | Planned / Needs Refactor            | 当前 `PdxRoute` 仍是局部 children matcher，组件 route module 与宿主 RouteGraph 合成尚未落地。                                                | 定义 RouteModule / RouteModuleMount，并让 `PdxRoute` 消费 route context。                                        |
...
| `13.route-runtime-contract.md`                      | In Progress             | Planned / Foundation Only           | Route runtime 字段已有规范方向，但 loader / guard / action 尚未通过 Code Authoring Environment 接入。                                        | 建立 RouteRuntimeContext 和 runtime CodeReference 链路。                                                         |
...
| `22.llm-integration-architecture.md`                | Draft                   | Foundation Only / Not Product-Ready | Gateway、context builder、tool registry、trace、streaming 与 provider 基础存在，但 AI action 仍偏 Blueprint/PIR，尚未覆盖 workspace target。 | 不升 Accepted；按 `ai-fine-grained-ui-actions.md` 建立 Workspace Action Proposal、dry-run/apply 与 repair loop。 |
