...
| `29.plugin-extension-points.md`                     | Draft                   | Partial（Contracts / Host / Palette / Phase 4.0-4.4） | Manifest/Palette/runtime/Gateway contracts、Host Core、Blueprint resolved Palette、Browser sandbox/Gateway，以及 workspace-scoped 单一 Web Plugin Platform 已落地。                                                                                   | 从 Phase 4.5 建立 exact official contracts 与 implementation registry，依次迁移 Ant Design、MUI，并在 compound/portal contract 稳定后迁移 Radix。 |
