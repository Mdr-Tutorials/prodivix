...
| `05.workspace-vfs.md`                               | Draft                   | Implemented / Stabilizing                  | Workspace VFS、VFS validator、`.prodivix/**` source projection 与 round-trip 测试已存在；ADR 35 已完成 canonical snapshot Hard Cut。                                                                                                                   | 补齐 Workspace Diff、Git history bridge 与实现边界后再升 Accepted。                                                |
| `06.command-history.md`                             | Draft                   | Implemented / Stabilizing                  | `WorkspaceCommandEnvelope`、`WorkspaceTransactionEnvelope`、原子 apply、History 与 reverseOps 校验已实现；编辑器作者态写入已完成 Hard Cut。                                                                                                            | 将 History 接入产品 undo/redo，并为 pending/outbox 提供稳定 command log。                                          |
| `07.workspace-sync.md`                              | In Progress             | Transport Implemented / Recovery Pending   | 后端 workspace API、document/route/VFS revision 链路、严格 wire codec 与 decoded mutation 已存在；legacy/fallback 写入已移除，但 durable outbox、rebase、冲突恢复和跨端同步尚未形成闭环。                                                              | 保持 In Progress；下一步实现 transport-neutral outbox、409 rebase/replay、失败恢复与冲突 UI。                      |
...
| `34.core-package-boundaries.md`                     | Accepted                | Phase 1 + Workspace Hard Cut Implemented   | `@prodivix/diagnostics`、`@prodivix/authoring`、`@prodivix/pir`、`@prodivix/workspace` 已建立；Web alias 删除；Compiler 复用 PIR materializer；ADR 35 已删除 Web Workspace 双模型。                                                                    | 下一批聚焦 Router owner package 与 `@prodivix/workspace-sync` outbox/rebase，再推进 Animation、Runtime/NodeGraph。 |
