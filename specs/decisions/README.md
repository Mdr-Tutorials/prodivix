...
| `29.plugin-extension-points.md`                     | Draft                   | Partial（内置侧收敛）           | 内置组件目录已收敛为第一方贡献：data/ → catalog/、options.ts 溶解、viewport 迁出；sampleData.tsx 保留为共享 demo 数据。                                                                                               | 完整插件宿主 / manifest / capability sandbox 待落地。                                                                                   |
