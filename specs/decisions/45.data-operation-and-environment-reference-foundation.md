...
- 日期：2026-07-15
- ImplementationStatus：Contract/Codec + Canonical Workspace/Semantic Foundation Implemented
...
现有 PIR `dataId` 保持文档内局部数据作用域语义。后续 PIR 纵切应新增由 PIR owner 持久化的 operation binding：binding 使用 `DataOperationReference` 指向全局 operation，把 execution result 投影到一个局部 `dataId`，再由现有 value binding 与 Collection 消费。

loading、empty、error 与 success 必须通过显式 lifecycle mapping 接入 Collection；不能通过检查返回对象的偶然字段或空数组形状来猜测。该 PIR binding、Collection lifecycle UI、operation trigger 与可逆 authoring transaction 尚未实现。
...
flowchart LR
    ...
    Consumer["Future PIR / NodeGraph / Code consumer"] --> Ref["DataOperationReference"]
    Ref --> Semantic
...
后续 protocol/runtime adapter 拥有外部网络协议与 provider execution；PIR 拥有 operation binding 和局部 data scope；Compiler 拥有 Preview/Export projection；`@prodivix/diagnostics` 拥有 Data diagnostic lifecycle、去重与 presentation。
...
- [ ] PIR operation binding、Collection lifecycle authoring 与可逆 transaction。
