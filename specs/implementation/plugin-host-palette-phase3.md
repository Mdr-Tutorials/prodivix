...
3. Native catalog 与当前 core-embedded external profiles 通过同一 Plugin Host contribution registry 发布。
...
### 4.2 Core-embedded external profiles

MUI / Ant Design 等现有 profile 继续由 ESM loader 取得真实 React component；这是 Phase 4 前的 runtime 过渡边界。它们生成的 Palette groups 以独立受信任 owner 发布：

```text
@prodivix/core.external.<libraryId>
```

配置重载时先按 owner disable Palette contribution，再清理 PIR runtime component；重新加载时先原子发布 Palette contribution，再注册画布 runtime。失败会进入 `ELIB-3010`，不再留下旧 Blueprint group 数组分支。
...
2. Native、headless 和 core-embedded external groups 进入同一个 owner-aware registry。
