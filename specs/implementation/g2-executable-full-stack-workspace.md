...
- ImplementationStatus：Foundation + S1 Neutral Snapshot/Remote Protocol/Control Plane/PostgreSQL/HTTP/Worker Agent/D2 Durable Ingestion/Rootless Sandbox/Preview-Build-Test Results/Artifact Resolver/Bounded HTTP Transport/Backend Auth Gateway Implemented / S1 Isolated Hosting/Provider Selection,S2-S5,S7-S8 Planned / S6 Decision-blocked
...
1. Renderer 与 standalone runtime 已能消费 `DataLifecycleSnapshot`；React/Vite 已从 exact runtime manifest
   隔离 mock 与 public client live，执行 query/mutation/policy 并发布 sanitized Network，但 Remote live、
   server/edge environment/Secret 和第二 target 尚未完成，不能把该纵切描述为完整 Data runtime。
...
状态：In Progress；invocation/registry/mock、exact document schema preflight、lifecycle stale fencing、
...
HTTP response mapping 和完整 live/policy runtime 继续建设。详细计划见
...
- [x] Remote Preview/Test/Build provider projection、result 与授权 resolver 使用同一 request/job/session/snapshot contract；HTTP hosting 与产品 composition仍待实现。
