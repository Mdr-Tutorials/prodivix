...
monotonic worker-attempt recovery，以及 Backend at-rest per-record KMS envelope、versioned static key ring、bounded atomic rewrap 与 legacy migration 均已完成。其他 permission、项目源码 mutation isolated target、managed-cloud KMS adapter 和完整 leak Gate仍按该计划继续。两条主线都不得把
...
任意项目源码 mutation、其他 permission、managed-cloud KMS adapter、跨 replica artifact/quota recovery、完整跨表面 leak Gate
