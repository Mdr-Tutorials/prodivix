...
- ImplementationStatus：Foundation + S1 Neutral Snapshot/Remote Protocol/Control Plane/PostgreSQL/HTTP/Worker Agent/D2 Durable Ingestion Implemented / S1 Production Sandbox,S2-S5,S7-S8 Planned / S6 Decision-blocked
...
| 能力                          | 当前基础                                                                                                                                | G2 剩余缺口                                                                                          |
| ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| Execution Core                | revision-bound Request/Provider/Job/Session、neutral snapshot，以及 versioned Remote codec/client、幂等/cursor/retry conformance 已实现 | 真实 transport/control plane、隔离 worker 与 Remote provider                                         |
| Browser Project Runtime       | React/Vite Preview、HMR、shared Browser Host、三画布 mode、Execution Center 与 neutral snapshot consumer 已实现                         | Terminal、Network、Remote Run                                                                        |
| Workspace Test                | Browser Test 与 Vitest adapter、canonical report、Test 页面已实现                                                                       | Remote Test provider、跨 provider conformance 与真实 transport smoke                                 |
...
| Data Authoring                | DataSource current/wire、Workspace typed document、Semantic contribution、PIR binding、Collection lifecycle transaction 已实现          | invocation、adapter registry、runtime kernel、mock/live、protocol adapter、真实 lifecycle            |
| Environment / Secret          | immutable reference shape 与 capability matching 已实现                                                                                 | resolver、permission、runtime-zone injection、redaction、environment store                           |
| Preview / Export              | React/Vite independent install/typecheck/test/build/browser Gate 已实现                                                                 | Data runtime requirement、zone partition、server gateway、CRUD parity、第二 target portability proof |
...
1. Renderer 与 standalone runtime 已能消费 `DataLifecycleSnapshot`，但当前产品没有生成真实
   snapshot 的 Data executor；“端口存在”不等于 Data runtime 已完成。
...
| ------------------------------------------------- | ------------------------------------------------- | -------------------- | -------------------------------------------------- |
| ExecutionProvider / Job / Session                 | ADR 40 / `g2-execution-provider-remote-runner.md` | Foundation           | Browser + Remote shared conformance                |
| Browser 与 Remote Isolated Runner                 | ADR 40、41                                        | Browser only         | exact snapshot digest 的 Preview/Test/Build parity |
| 文件、依赖、HMR、Console、Terminal、Network、Test | ADR 41、44                                        | Partial              | provider-neutral Execution Center journey          |
| Data/API IR 与 lifecycle policies                 | ADR 45                                            | Contract only        | deterministic Data runtime policy suite            |
| HTTP/OpenAPI、GraphQL、AsyncAPI adapter           | ADR 45                                            | Not Started          | importer/runtime fixture conformance               |
| client/worker/server/edge/build/test zones        | ADR 40、45                                        | Static identity only | compatibility + authorization matrix               |
| SecretRef、environment、permission、mock/live     | ADR 40、45                                        | Reference only       | canary leak test + real authorized runner          |
...
| `@prodivix/runtime-browser`    | WebContainer/browser project、client-safe adapter、Browser Host；不拥有 Remote 或 Data domain semantics                                                                                                    |
| `@prodivix/data`               | Data current model、invocation、adapter registry、lifecycle/policy kernel、protocol-neutral importer mapping                                                                                               |
...
状态：In Progress；neutral snapshot Hard Cut 与 Remote protocol/client 已实现，真实 control plane、worker 和 providers 尚未实现。详细计划见 `g2-execution-provider-remote-runner.md`、
...
状态：Planned。详细计划见 `g2-data-operation-environment-runtime.md`。
...
状态：Planned / resolver ownership decision required。
...
- [ ] Remote Preview/Test/Build 使用同一 request/job/session/snapshot contract。
- [ ] Terminal、Network、Console 与 Test 无私有 execution truth。
- [ ] Data invocation、adapter registry、policy kernel与 mock/live runtime 完成。
