...
| 文件、依赖、HMR、Console、Terminal、Network、Test | ADR 41、44、49                                    | provider-neutral Console/Network/Test、Remote PTY/FS、细分 recovery 与 Server/Data SourceTrace navigation implemented          | 更多 producer 的统一 debugger/recovery closure                                     |
| Data/API IR 与 lifecycle policies                 | ADR 45、49                                        | Canonical authoring、finite runtime 与 bounded stream foundation implemented                                                   | reconnect/resume、credential renewal 与 incremental collection semantics           |
| HTTP/OpenAPI、GraphQL、AsyncAPI adapter           | ADR 45、49                                        | HTTP/OpenAPI、finite GraphQL/AsyncAPI 与 public GraphQL subscription/AsyncAPI SSE/NDJSON pull stream implemented               | 更多 transport、resume 与 Secret-authenticated stream Gate                         |
| client/worker/server/edge/build/test zones        | ADR 40、45                                        | Authorization first vertical                                                                                                   | compatibility + authorization matrix                                               |
| SecretRef、environment、permission、mock/live     | ADR 40、45、46                                    | Production + isolated sealed Secret + exact workspace.read composition + Backend KMS/AWS adapter local vertical                | A14/A15 remote evidence + complete cross-surface leak Gate                         |
| Binary Asset pipeline                             | ADR 47 + `g2-binary-asset-pipeline.md`            | B0-B5 local/cloud/transform/delivery + B6 retention/Git/runtime import + B7 Browser journey + real-daemon evidence implemented | multi-vendor/full-raster/more-format/public delivery/cross-target journey Gate     |
| Auth/session/permission/server function           | ADR 46 + `g2-auth-server-runtime.md`              | A0-A12 implemented；A13-A16 configured / evidence pending                                                                      | A13/A15/A16 rootless + A14 AWS evidence + other permission/role/full-stack closure |
| 单一第二 framework portability proof              | ADR 31、48 + 本计划                               | Controlled Vue/Vite Data/Executable/Remote first vertical implemented                                                          | protocol/target matrix + independent install/typecheck/test/build/Chrome CRUD Gate |
...
runtime filesystem Asset import/replace、B7 Browser JPEG 产品旅程和 real-daemon 首次远端通过证据。
...
monotonic worker-attempt recovery，以及 Backend at-rest per-record KMS envelope、versioned static key ring、bounded atomic rewrap 与 legacy migration 均已完成。A13 已完成 `workspace.write` 单文件项目源码 mutation，A14 已配置 AWS managed KMS，A15 已完成 `workspace.read` authority + one-shot Secret 的本地组合纵切，A16 已完成 canonical viewer role 到 exact read-only durable/Worker authority 的本地纵切；新增 GitHub/AWS evidence待取得，其他 permission/role、第二 managed KMS provider 和完整 leak Gate仍按该计划继续。两条主线都不得把
...
proposal/reimport/Workspace adoption、finite GraphQL/AsyncAPI、public GraphQL subscription/AsyncAPI SSE/NDJSON
...
exact-snapshot SourceTrace navigation 已实现。stream reconnect/resume、Secret credential renewal、更多 transport、
incremental collection semantics 与真实 Remote authenticated cross-target journey 继续 fail closed 或建设。
...
regional DR 首次真实云端 RPO/RTO evidence、首次 live MRK evidence 与其余 producer debugger closure 继续建设。
...
状态：In Progress；Auth/Server A0-A12 已实现，A13 source mutation、A14 AWS KMS、A15 read + Secret 与 A16 collaborator viewer read authority 已完成本地纵切和 rootless/PostgreSQL/OIDC 配置、远端证据待取得；
...
其他 permission/role、其他 managed-cloud KMS adapter、A13/A15/A16 新增 rootless evidence、A14 live AWS evidence、跨 replica artifact/quota recovery、完整跨表面 leak Gate
与独立 full-stack product journey 仍待完成；不得把 execution-state 安全 adapter、isolated Secret code-export 或单文件 staging mutation
...
产品 target selector 与 authenticated Catalog CRUD + PNG Chrome Gate 由 ADR 54 完成。真实 Remote authenticated
live Catalog、layout/outlet 与完整 Asset delivery/sanitize UI matrix 继续建设。
...
状态：Authenticated Vue Catalog Local Product Gate Implemented / G2 In Progress；living Golden snapshot、
Browser/Remote contract matrix、真实 rootless Preview/Test/Build、second-target CRUD/lifecycle，以及 Vue deterministic
authenticated PIR/Route/Server/Asset Chrome journey 已建立。真实 Remote authenticated live Catalog、完整 permission
denial/Asset delivery matrix 与跨产品面 closure 继续建设。
...
- [x] React/Vite 与 Vue/Vite 通过同一 mock CRUD runtime、protocol/target snapshot matrix 与独立
      ...
      Catalog CRUD + PNG Chrome Gate 已完成，真实 Remote authenticated live journey 继续建设。
