...
- Phase 4.0-4.5 implemented; Phase 4.6-4.9 planned
- 日期：2026-07-10
- 前置阶段：
...
External Library、Render Policy、Codegen Policy 与 Icon Provider 已进入同一 TypeScript map，并在 Phase 4.5 完成 production JSON Schema、生成类型、validator、resolver、跨点批次校验与 revisioned projection snapshot。Renderer、Compiler 和 Icon surface 消费同一 Host registry 的投影，没有创建平行 contribution Host。当前产品 Gateway 只注入稳定的 Workspace summary 与 Document read ports，intent/patch write port 在精确业务 contract 冻结前保持 unavailable。
...
- [x] 将 `iconProvider` 加入 built-in point catalog；Radix 前再加入独立版本化的 `blueprintTemplate` contract。
