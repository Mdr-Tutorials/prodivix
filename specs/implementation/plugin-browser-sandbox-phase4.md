...
- Phase 4.0 implemented; Phase 4.1-4.9 planned
- 日期：2026-07-10
- 前置阶段：
...
6. 用一个 workspace-scoped Web Plugin Platform 承载 Palette、External Library、Render Policy、Codegen Policy 与后续扩展点，不再保留 Blueprint Palette 私有 Host。
...
- Native、headless、MUI 和 Ant Design Palette groups 已进入同一个 resolved Palette registry。
...
### 2.4 当前 Web composition root 只是 Phase 3 过渡实现

`apps/web/src/editor/features/blueprint/palette/host.ts` 当前：

- 在 module scope 创建只认识 `paletteContribution` 的 Host。
- 只对 `core` trust level grant registration capability。
- 明确拒绝所有 runtime activation。
- 使用内存数组保存最多 1,000 条 audit event。

它适合验证 Phase 3 Palette，但不能承载一个同时贡献 Palette、External Library、Render Policy 与 Codegen Policy 的真实插件。Phase 4 必须将其替换为 workspace-scoped Web Plugin Platform；不能在每个 surface 再创建一个 Host。

### 2.5 当前外部库仍有库专属 core 分支
...
Palette 已经走 Host 不等于这些 runtime/render/codegen 路径已经插件化。
...
后续 point 在 composition root 显式加入。不得为 Renderer、Compiler 或 Inspector 创建平行 Host。

Web composition 同时注册一个与 Host contracts 对齐的 `OfficialHostImplementationRegistry`。它是 build-time trusted implementation catalog，不是第二个 contribution registry；所有实现必须由 Host contribution owner 引用并随 owner cleanup。

### 11.2 Lifetime

- Web Plugin Platform 由 workspace/editor shell 创建，不在 feature module import 时创建。
- workspace switch、logout、test teardown 和 HMR replacement 都调用 `shutdown`。
...
- surface 只注入 typed reader/query service，不持有 Host mutation capability。
- registry snapshot revision 是 render、Palette、Inspector 与 compiler policy projection 的一致性边界。

### 11.3 Phase 3 adapter 迁移

`apps/web/src/editor/features/blueprint/palette/host.ts` 的职责拆分到：

- Web composition root：Host、policy、runtime adapter、audit、ID/clock。
- Palette contract module：Palette validator/resolver。
- trusted core package adapter：构造 `@prodivix/core` package source。
- Palette query service：把 Host records 投影为 Blueprint snapshot。

迁移完成后删除 Palette 私有 Host 和第二层 `runSerialized`。per-plugin serialization 由 Host operation coordinator 负责，surface 不应重复实现另一套并发协议。
...
所有 Schema 都保持 closed object、JSON-only、大小上限和 exact contract version。`@prodivix/plugin-contracts` 继续从 Schema 生成类型/runtime Schema，并实现 point-specific semantic validator。

### 12.2 External Library descriptor

插件声明 package coordinate、exact version、export discovery、Canonical External IR enhancement 与 dependency metadata，不声明任意 executable URL。
...
新增 `packages/plugin-antd/`，包含可发布 package 边界、Plugin Manifest、contribution JSON 与必要的 package metadata。Web 只通过 generic bundled-official package catalog 发现它，不导入 `antdProfile` 或 AntD 专属 resolver。
...
Ant Design 完成后再迁移 MUI，目的在于证明 contract 是库无关的：
...
当前 Radix Palette/renderer 仍以 placeholder 为主。Radix 不能仅把 `RadixAccordion` 映射成一个 `div` 就宣称迁移完成。
...
3. Canonical External IR 提供 slot/cardinality/context 语义，能表达 Root/List/Trigger/Content 等 compound component。
...
门禁通过后新增 `packages/plugin-radix/`，删除 `apps/web/src/pir/renderer/registry.ts` 中 Radix placeholder 和 React compiler 中 Radix fallback。未通过前保持“planned”，不制造半真实 catalog。
...
- [ ] 新增 runtime/control/implementation/gateway JSON Schema。
- [ ] 创建 `@prodivix/plugin-protocol` 并配置 generated/check:generated。
- [ ] 实现 strict JSON text codec、byte/depth/node limits 与 exact method registry。
- [ ] 实现 request correlation、sequence、cancel 与 timeout state machine。
- [ ] 新增 protocol diagnostics，并同步 `specs`、definitions 与 `apps/docs/reference/diagnostics/`。

Gate：对 malformed、duplicate key、unknown method/version、replay、late response 和 oversized message 的测试全部 fail closed。

### Phase 4.2：Opaque sandbox 与 Browser adapter

- [ ] 输出独立 broker HTML/bootstrap 与受 digest 约束的 Dedicated Worker bootstrap bytes。
- [ ] 配置 production headers、CSP、Permissions Policy 与 no-cookie sandbox origin。
- [ ] 实现 cryptographic bootstrap nonce、private MessagePort 和 exact handshake。
- [ ] 实现 verified runtime import、heartbeat、termination 与 runtime session。
- [ ] 实现 runtime/UI 两套 sandbox policy；UI 先完成 conformance fixture。
- [ ] 将 Browser adapter 接入 Host Core runtime port。

Gate：Playwright 在真实 production build 中证明插件不能访问 parent DOM、network、storage、top navigation 或 nested Worker，runtime hang/crash 不影响 editor 主循环。

### Phase 4.3：Gateway、quota 与持久化 audit

- [ ] 实现冻结的 Gateway contract registry 与 dispatcher。
- [ ] 接入 live permission、Manifest request、owner/generation 和 cancellation guard。
- [ ] 实现 per-session message/rate/concurrency/timeout policy。
- [ ] 实现 host-side persistent bounded audit store、redaction 与 sensitive preflight fail-closed。
- [ ] 实现 Phase 4 最小 handler 集合；`secrets.read` 保持 deny。
- [ ] 实现 network allowlist/redirect/header/body/response policy。

Gate：撤权 mid-flight、audit unavailable、redirect escape、quota flood 与 stale generation 均无 side effect 或残留 handle。

### Phase 4.4：统一 Web Plugin Platform

- [ ] 建立 workspace-scoped Web composition root 与 React provider/service injection。
- [ ] 注册 Palette、External Library、Render Policy、Codegen Policy 与 Icon Provider contracts。
- [ ] 将 `@prodivix/core` native contribution 迁入新 Host session。
- [ ] 将 Palette reader/query 注入 Sidebar、建节点与 Inspector。
- [ ] 删除 Blueprint Palette module singleton、私有 policy/audit 和重复 serialization queue。
- [ ] workspace switch/HMR/test teardown 统一调用 Host shutdown。

Gate：同一 plugin 的多 point contribution 在一个 owner/generation transaction 中发布或整体回滚；Web 中只有一个 active Host session。

### Phase 4.5：External/Render/Codegen/Icon contracts

- [ ] 新增四个 Schema、生成类型、validator、resolver 与 diagnostics。
- [ ] 将 `iconProvider` 加入 built-in point catalog；Radix 前再加入独立版本化的 `blueprintTemplate` contract。
- [ ] 用 Host-controlled package coordinate resolver 替换 plugin-provided executable URL。
- [ ] 从 Render Policy 生成 host-side runtime adapter/projection。
- [ ] 建立 build-attested Official Host Module catalog，并按 owner/generation 绑定 host implementation id。
- [ ] 从 Codegen Policy snapshot 生成 compiler composite adapter。
- [ ] 将 icon renderer/codegen 迁到 Icon Provider registry。
- [ ] 补跨 point identity、library ownership、runtime type 与 implementation reference semantic validation。

Gate：fixture plugin 不写任何 library-specific Host branch，即可完成 Palette -> Canvas -> React export。

### Phase 4.6：Ant Design official plugin

- [ ] 创建 `@prodivix/plugin-antd` package artifact。
- [ ] 迁移 Palette、external descriptor、render/codegen policy 与 icon provider。
- [ ] 通过 official package source 完成 discover、grant、register、use、disable、update 与 shutdown。
- [ ] 删除全部 Ant Design core 专属实现和测试替身。
- [ ] 补完整 editor smoke 与 export behavior test。

Gate：禁用/移除插件后 Palette、runtime registry、render policy、icon provider 与 compiler snapshot 中均无 Ant Design lease；重新启用只产生一个新 generation。

### Phase 4.7：MUI official plugin

- [ ] 创建 `@prodivix/plugin-mui`，只使用 Phase 4.5 contract。
- [ ] 迁移 Palette、runtime、render/codegen policy 与 icon provider。
- [ ] 回归 Ant Design，证明通用 contract 未被 MUI 特例污染。
- [ ] 删除全部 MUI core 专属实现。

Gate：Host/Web/Compiler 不出现 MUI library-id 分支；Ant Design 与 MUI 可独立 enable/disable 且 snapshot revision 一致。

### Phase 4.8：Radix contract 与真实编辑器闭环

- [ ] 冻结 compound template、slot/cardinality 与 portal-safe policy contract。
- [ ] 通过 Command/Intent 实例化 normalized multi-node fragment。
- [ ] 创建 `@prodivix/plugin-radix` 并覆盖基础、compound 与 portal 组件。
- [ ] 删除 Radix renderer placeholder 与 compiler fallback。
- [ ] 验证 Canvas interaction、Inspector、建节点、preview 和 React export。

Gate：Accordion/Tabs/Dialog/Tooltip 使用真实 Radix runtime/codegen，不是 `div` placeholder，且 portal cleanup 不残留 DOM/overlay handle。

### Phase 4.9：Hardening、文档与状态收口

- [ ] 完成 threat-model security test、protocol fuzz/property test 与 browser matrix。
- [ ] 验证 production sandbox headers 和部署配置，不只验证 Vite dev server。
- [ ] 运行 package tests、web tests、typecheck、lint、build、diagnostics generation 与 VitePress build。
- [ ] 运行 `pnpm run format`，接受 formatter 自然产生的格式变化，不手工逆格式化。
- [ ] 同步 ADR 14/17/29、实现文档、PLG diagnostics 与 `apps/docs`。
- [ ] 仅在所有 exit criteria 成立后更新 ADR 29 implementation status。
...
Phase 4 按 4.0 至 4.9 的 gate 串行推进。安全底座未通过前不开始 official plugin 删除；Ant Design 未完成直接替换前不开始 MUI 特例；compound/portal contract 未通过前不宣称 Radix 可用。
...
- [ ] 普通 same-origin Worker 不被文档或代码当作安全边界。
- [ ] runtime 在 opaque/cross-origin broker + Dedicated Worker 中执行，并由 production CSP/Permissions Policy 约束。
- [ ] runtime artifact 与 manifest/package/owner/generation 显式绑定并在执行前验证。
- [ ] protocol、Gateway 和 implementation payload 全部经过 strict JSON、Schema、size 与 exact-version validation。
- [ ] 未请求、未授权或已撤销 capability 无法调用 Gateway。
- [ ] plugin 不能直接访问 parent DOM、network、storage、Workspace store、PIR 或 compiler internals。
- [ ] hang、crash、quota violation、disable、update 与 shutdown 均完成 owner cleanup。
- [ ] lifecycle audit 与敏感 Gateway audit 有明确 best-effort/fail-closed 边界。
- [ ] 同一 workspace 只有一个 Web Plugin Host session 和一个 typed contribution graph。
- [ ] Ant Design 与 MUI 已作为 official plugin 运行，core 中对应 profile/render/codegen/icon 分支已删除。
- [ ] Radix compound/portal contract 验收通过，并在 Blueprint 中使用真实 runtime/codegen 而非 placeholder。
- [ ] compiler 只消费显式 serializable policy snapshot，不读取 browser registry singleton。
- [ ] PLG definitions、generated docs、ADR、implementation docs 与真实代码同步。
- [ ] format、targeted tests、full tests、typecheck、lint、production build、diagnostics check 与 docs build 全部通过。

Phase 4 完成后，Phase 5 才进入 public SDK、模板、签名/审核、marketplace、GSAP/react-spring 等更多 official plugin family 与社区生态。
