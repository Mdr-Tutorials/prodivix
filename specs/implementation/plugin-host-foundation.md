...
└── examples/
    └── plugin-antd.manifest.json
```

后续扩展点 Schema 采用独立文件：

```text
specs/plugins/contributions/
├── palette-contribution-v1.schema.json
├── codegen-policy-v1.schema.json
├── diagnostic-provider-v1.schema.json
└── ...
```
...
apps/web/src/plugins/browser/    # Browser sandbox transport 与 Web Host Gateway
```
...
- [x] core-embedded external profile 接入同一 registry。
