...
- Backend 从 exact Workspace owner 或 A16 canonical viewer role解析后，于 authenticated create 的受信 transport header 中投影短期
  `providerId + principalId + sorted allowed permissions + Workspace + snapshot + expiry` attestation；permission list
  严格去重、最多 32 项；owner只签发有界 `workspace.owner/read/write`，viewer只签发 `workspace.read`。TTL 默认 2 分钟、最大 5 分钟且不晚于
...
vertical + Remote Preview/isolated production Invocation Devtools first vertical Implemented；完整产品闭环继续建设。
...
- ADR 54 的 Vue current-contract target 复用同一 Auth config、Route guard/loader/action 与 deterministic
  ...
  exact PNG。Remote 单元格当前只声明 source-free snapshot/capability/codec closure，不冒充真实 live effect。
...
状态：Implemented；A10 已另行关闭 bounded cross-worker-attempt recovery，A11 已关闭 static KMS/key rotation，A12 已新增 Secret-free `workspace.read`，A13 已配置独立 source mutation，A15 已配置 `workspace.read` + Secret 组合 Gate，A16 已配置 viewer exact read authority；其他 permission/role 与完整 leak closure 继续关闭。
...
rotation 与 legacy migration first vertical。A14 已另行配置 AWS managed KMS adapter；完整跨表面 leak closure、其他
permission 和项目源码 mutation 不复用 KMS/Secret 通道，A13 只在无 Secret 的独立 isolated policy 下开放。
...
由 A16 承接；editor/write collaborator role与其他 permission继续关闭。
...
状态：Configured / Evidence pending；canonical contract、作者态、Compiler、Worker、Golden、本地 aggregate 与
GitHub rootless real probe 已实现，新增真实远端 rootless run 尚未取得。
...
- Backend 对 write profile 仍只有 Workspace owner permission set 能投影排序后的 `workspace.write`；A16 viewer set不含 write。
...
- rootless contract Gate 已验证 exact plan/authority/staging SourceTrace；真实 Podman probe 已配置验证 network-none runtime、
  Secret absent、唯一 whole-file modified diff、transport exclusion、trusted result 与 cleanup。取得新增 GitHub run 前，milestone
  保持 `Configured / Evidence pending`。
...
状态：Configured / Evidence pending；shared policy、Backend broker、Compiler/runner、Worker、reference-only authoring、
Living Golden 与 rootless real probe 已实现并通过本地 Gate，新增真实 GitHub rootless run 尚未取得。
...
- Living Golden 新增独立 `isolated-workspace-read-secret-code-export` 列：Browser/static、deterministic Test 与 Remote live blocked，
  ...
  取得新增 GitHub run/evidence前保持 `Configured / Evidence pending`。

### A16：Workspace collaborator viewer exact execution authority first vertical

状态：Configured / Evidence pending；canonical role persistence、Backend resolver、durable principal/permission grant、
Data/Server/Secret effect recheck、真实本地 PostgreSQL Gate 与 read-only rootless contract/real probe 已完成；改动后的
GitHub rootless run 尚未取得。
...
- rootless `workspace.read` runner/fixture 从 owner-derived三项 authority收紧为真正的单项 read authority，并在 snapshot contract
  证明该 authority不能执行 `workspace.write` mutation。`.github/workflows/g2-postgres.yml` 新增真实 role grant/resolve/create/
  durable permission/revoke Gate；`pnpm run verify:g2:isolated-server-collaborator-read` 是本地汇总入口。首次更新后的 GitHub
  rootless evidence取得前保持 `Configured / Evidence pending`。
...
- isolated collaborator viewer read Gate：`pnpm verify:g2:isolated-server-collaborator-read`
- collaborator viewer PostgreSQL Gate：
  `go test ./internal/modules/remoteexecution -run '^TestWorkspaceExecutionViewerRolePostgreSQLGate$' -count=1 -v`
...
- [ ] A13 新增真实 GitHub rootless run 与 evidence upload完成。
- [ ] A14 新增真实 AWS OIDC/KMS run、A15/A16 新增真实 rootless run 与 evidence；其他 managed KMS provider、完整 leak closure与其他 permission完成。
