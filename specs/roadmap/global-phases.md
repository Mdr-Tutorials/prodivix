...
| G2           | In Progress       | Execution + Data Authoring Foundation     | ExecutionProvider/Job、neutral snapshot、Browser Providers、Remote codec/client/provider projection/Preview Bundle/Build Bundle/Test Report result、授权 artifact resolver、有界 HTTP transport、Backend auth gateway/durable execution grant、Control Plane/PostgreSQL/HTTP、Worker Agent、rootless Podman sandbox/GitHub Gate、短期 capability Preview Host、Blueprint Browser/Remote selection、Golden contract matrix、真实 Golden rootless Preview/Test/Build、install/runtime 隔离、install allowlist proxy、Browser fetch/Data HTTP adapter、operation correlation 与 sanitized Network 产品视图已建立；Data 作者态、typed trigger/input dispatch、PIR v1.6 trigger/input authoring、React/Vite generated mock CRUD 与 public client live HTTP/schema/retry/pagination/cache/optimistic runtime、显式 mock/live manifest、strict iframe Network bridge、invocation/registry/lifecycle execute kernel，以及 environment resolution/Secret permission first vertical 已落地；Backend production Environment/Secret store、principal/session partition、durable grant/audit、per-record KMS data-key envelope、versioned static key-ring、bounded atomic rewrap/legacy migration、Remote create exact environment/session/snapshot authority binding、execution-bound Remote HTTP/material query/mutation gateway、effect-before durable replay fence/真实 PostgreSQL concurrency Gate，以及 generated Remote Preview server/edge query/mutation value-only bridge/exact capability-origin CSP 已建立；finite Remote Preview 后续 Data Network 与 Structured Console 通过 exact active-job、generation-fenced、bounded Session observation 进入产品视图；React/Vite target manifest 已使 server/edge Data 只有显式 execution parent gateway 才能编译，并将 `network`/`environment-binding` 传播到 snapshot/provider/request，Browser/ZIP export fail closed；Remote mutation 已支持显式 `invocation-key` upstream idempotency、opaque HTTP header 与 v3 next-attempt ledger；Remote 当前 durable 输出的 request/snapshot/cache/log/diagnostic/trace/artifact/test-report/crash canary 已由 Worker/Control Plane 双 Gate 覆盖；Structured Console 双预算投影、generated/bridge/core/copy credential redaction 与 manual new-request recovery 已建立；Remote Preview Terminal 的 grant/lease/cursor/幂等/双预算/copy、短期 token rotation、Backend owner gateway、Control Plane ephemeral broker、rootless inner PTY、跨 chunk canary、reconnect 与 execution cleanup 已建立，Browser 显式 unsupported；runtime FS strict diff/whole-file CodeArtifact add/modify/delete VFS proposal 已建立；完整 Remote reconnect/artifact/quota/worker-loss recovery、managed-cloud KMS 与第二 target 继续建设 |
...
- instance-owned `ExecutionSessionCoordinator` 以稳定 session identity 组合 revision-bound Jobs，提供有界事件/trace/Console observation 保留、订阅、清理与取消入口；它不持久化 Workspace，也不拥有 provider process。
...
- 蓝图画布已形成 Design / Interactive / Run 三模式；Run 通过 sandboxed iframe 原位显示真实项目 server，共享 Execution Center 提供 Structured Console 过滤/安全复制、Network、停止、manual new-request restart、刷新和独立预览。取消在旧 Job terminal 前保持 `cancelling`，旧事件有界保留且 mutation 不自动重放。
...
- `@prodivix/data` 已建立无版本号 current DataSourceDocument、DataOperationReference、JSON Schema 2020-12 shape、query/mutation、pagination/cache/retry/idempotency/optimistic policy、lifecycle contract、strict normalization 与 `wireVersion: 1` codec，并新增 transport-safe invocation、typed trigger origin/input mapping、deterministic dispatch coordinator、mock/live-aware adapter registry、exact DataSourceDocument input/output schema preflight、instance-owned duplicate/stale/superseded lifecycle fencing、deterministic retry scheduler/fixed-exponential backoff、attempt lifecycle/Network correlation、offset/cursor pagination input/default/page fencing、SHA-256 partitioned bounded cache、owner/version fenced optimistic CRUD projection，以及无 idempotency contract 的 mutation replay denial；query input-change 使用 canonical equality，mutation 只接受 Blueprint event/CodeSlot/Test 显式 dispatch identity，duplicate 与 disconnect-after-dispatch replay 不会再次执行。cache key 绑定 operation/document revision/canonical selected input/environment/runtime/adapter/target/principal partition，Secret 不入 key，缺安全 principal partition 的鉴权 query 强制 no-store，SWR stale hit 只发布显式新 sequence revalidation signal。optimistic create/update/delete 以显式 input/output/entity RFC 6901 mapping 绑定 target partition，成功按 authoritative response reconcile，错误/取消只在 invocation 仍拥有版本时回滚；旧 inverse patch 不覆盖新 mutation 并要求 revalidation。独立 `@prodivix/data-http` 通过 Browser client-safe fetch 完成 live HTTP 第一纵切，并声明 `idempotency-key` capability；`@prodivix/data-mock` 已提供 session-scoped deterministic fixture、exact/fallback matching、delay/error/page、stateful CRUD namespace、reset/dispose fencing 与 Browser Test mock-default/live-opt-in composition。Browser Data environment 已组合 typed dispatch 与共享 execute kernel。Executable Project Snapshot v6 已同时纳入有界、content-addressed `dataMockProvision`、immutable initial collections 与 execution-only `serverRuntimeMockProvision`，Compiler、Remote strict codec 与 Browser/Remote Test 使用同一 exact provision；Server Runtime fixture 仅在 Test 投影，Preview/Build 得到不含 fixture value 的 disabled envelope。React/Vite standalone 已生成 PIR Data lifecycle subscription，并执行 document/route/input-change activation、semantic typed input、Blueprint mock mutation CRUD 与已激活 query revalidation，以及 public client live HTTP/schema/retry/pagination/cache/optimistic/idempotency runtime；PIR-current v1.6、Inspector、Semantic/CodeSlot 与 Workspace 原子事务也已完成 durable authoring。Remote server/edge query/mutation 已接 execution-bound gateway；mutation 使用 runtime-unique invocation、durable replay fence、attempt-invariant opaque upstream key 与 v3 next-attempt ledger，第二 target 仍未完成。
...
- `@prodivix/runtime-core` 已提供 reference-only ExecutionEnvironmentSnapshotRef、EnvironmentBindingReference 与 SecretRef，并允许 ExecutionRequest 绑定 immutable environment snapshot identity；新增 transport-neutral snapshot/permission/material ports 与短期 resolution lease，exact revision/mode/binding、adapter field、runtime zone、execution class、provider isolation、principal/session、expiry/revoke 均在 effect 前校验，并为鉴权 Data cache 派生不可逆 session partition。Data/HTTP first vertical 可解析 public configuration，并只在授权 isolated server transport callback 注入 Secret；lease metadata、audit、result 与 Network trace 的 canary 测试不含明文。Backend 已建立 immutable revision、per-record AES-256-GCM data-key envelope、versioned static KMS key ring、bounded atomic rewrap/legacy migration、durable exact grant/audit 与 API/at-rest/callback canary 的 production store first vertical；Remote create 已在转发前 preflight exact environment/session/snapshot authority。execution-bound Remote HTTP/material gateway query/mutation first vertical 只读取 exact Data document revision，以短期 grant 在公网 HTTPS callback 内注入 Secret，并输出 metadata-only Network trace；generated Remote Preview 以 value-only strict request、exact opaque frame/generation fence、父窗口 product-authenticated client 和 exact capability-origin CSP 接入该 gateway，Secret/environment identity 不进入 iframe、生成源码或 Preview Host。mutation 在 effect 前 durable claim，completed duplicate 返回 sanitized result，identity drift/pending/indeterminate fail closed；显式 `invocation-key` + safe public HTTP header mapping 时，v3 ledger 只把 retryable outcome 原子释放给紧邻下一 attempt，所有 attempt 复用 opaque SHA-256 key；无 contract 时 attempt 固定为 1。Remote Network 已通过 strict response 与 exact active-job Session observation 进入产品视图，finite terminal Job 不被重新打开，旧 generation/stop 后的结果 fail closed。React/Vite server-gateway compile Gate 已通过显式 target manifest、Remote capability propagation、Browser/ZIP denial 与生成 source/diagnostic/snapshot identity canary 落地；managed-cloud KMS adapter 与全表面 leak suite 仍待实现。
...
- transport-neutral Data trigger origin、typed input mapping、query activation equality、mutation dispatch identity replay fencing、Browser composition、PIR/Inspector v1.6 durable trigger authoring，以及 Preview/Export shared React/Vite mock trigger/CRUD runtime、standalone public client live/policy runtime、Backend production Environment/Secret store + KMS envelope/key rotation first vertical、Remote create authority binding、execution-bound HTTP/material query/mutation gateway、durable replay fence、显式 upstream idempotency/retry、generated Remote Preview query/mutation bridge/CSP、Remote Network/Console Session correlation、manual new-request recovery、Structured Console copy Gate、Remote Terminal transport-wide/cross-chunk Gate、runtime FS diff/whole-file CodeArtifact add/modify/delete VFS proposal 与当前 Remote durable 输出 canary Gate 已完成；完整 Remote failure recovery、managed-cloud KMS、OpenAPI/GraphQL/AsyncAPI adapter，以及第二 target CRUD parity 尚未完成。
- ADR 46 已冻结 AuthPrincipal、server-only session reference、permission port、Server Function code profile/invocation/outcome 与 target matrix。`@prodivix/server-runtime` authorization/schema kernel、session-bound exact-revision Backend gateway、current-principal loader、workspace-owner guard、Remote value-only iframe bridge、React/Vite guard/loader projection、`server-function` capability propagation和 Browser/ZIP fail-close已完成 first vertical；deterministic Auth Test 已通过 Snapshot v6 provision、strict Remote codec、default built-in Auth fixture、typed Route action、invocation-key replay/conflict、navigation/iframe/HTTP cancel 与 loader revalidation形成第二纵切。Isolated production 已通过 Snapshot v6 production plan、128-module/64-depth/4 MiB bounded canonical TS/JS import graph、deterministic `.mjs` projection、per-module/aggregate SourceTrace、独立 Remote provider、rootless networkless Worker 与可信结果二次校验，并以 Backend 短期 attestation、Control Plane atomic authority row、worker-attempt lease 与 one-shot runner principal/`workspace.owner` permission projection 完成 authenticated/permission read/guard。A12 又以 shared Compiler/Worker policy、owner-derived sorted authority、Resources/Blueprint authoring、Living Golden 与独立 rootless probe完成 Secret-free `workspace.read` isolated permission first vertical；单独 owner grant 不替代 read grant，`workspace.read` + environment继续 fail closed。permission list 采用 sorted/unique/32-item budget，Control Plane idempotency identity 与 Worker attempt 均绑定 grant，generated runner 在项目 effect 前校验并删除 authority。Remote live mutation 已为唯一 `core.server.execution-state.put` adapter 完成 exact configured Origin、自定义 intent header、credential canary、execution/function partition、SHA-256 invocation identity、PostgreSQL effect/replay 单事务及真实并发/容量/cascade Gate。Remote live Secret 已为审计 `core.server.hmac-sha256` adapter 完成 reference-only field -> SecretRef profile、callback-bound kernel、exact execution/principal/session/environment/function/invocation grant、30 秒 IssueGrant/UseSecret/Revoke、canonical JSON HMAC、`environment-binding` capability propagation与 material/output canary；Browser/static、deterministic Test、isolated production及其他 adapter 保持 fail closed。Living Golden Auth/Server target matrix 已显式覆盖 Browser/static fail-close、canonical owner/read adapter 的 deterministic Test、审计内置 Remote live owner/HMAC gateway projection、isolated owner/read/Secret code-export 真实 production 执行、one-shot authority、correlation、root/helper SourceTrace 与 credential/source boundary；Route 作者面已从 canonical profile 投影候选/binding/issue，复用可逆 intent bind/unbind，并以单个 Workspace Transaction 创建 Remote/isolated owner/read-guard CodeArtifact + binding，经 Blueprint Inspector、Issues 和 wire reload -> compile Golden Gate 验证。`/config/auth.json` strict reference-only provider/permission contract、可逆 Workspace authoring、Resources/Issues、Remote/isolated compile Gate 与 Golden reload first vertical 也已完成。Remote Preview 与 isolated production invocation completion 现共同使用 strict metadata-only `server.function` contract；前者绑定 exact generation/Session/Job observation，后者在 canonical artifact upload 后发布 durable trace，并由 Remote provider 强校验 artifact/trace correlation。Execution Center Server 只对唯一 root CodeArtifact 提供 exact Workspace snapshot SourceTrace navigation；input/output/credential/source 不进入 detail，Browser 不伪造 producer。第三方 Auth provider、`workspace.write`/其他 permission、项目源码 mutation、managed-cloud KMS、Remote Test/后续 producer debugger 与完整跨表面 leak Gate 仍待完成。
- A9 已在上条 A7/A8 边界上关闭 dedicated isolated Worker Secret resolution first vertical：仅允许
  `prodivix.code-export + read + server + public|authenticated|workspace.owner`，以 exact execution/snapshot/
  function/invocation/worker-attempt claim、Control Plane active lease、Backend 30 秒 `remote-isolated/isolated-runner`
  grant、X25519 临时 recipient、HKDF + AES-256-GCM envelope、PostgreSQL ciphertext-only one-shot replay、
  install 后 residual-process cleanup、reserved `.prodivix` directory 删除重建与 canonical transport-path hard cut、
  rootless mode 0600 material、effect 前 delete 与 `useSecret` callback完成端到端执行。Browser/static、Test、
  Remote live code-export、其他 adapter/permission 与 mutation保持 fail closed；KMS/key rotation、Remote Test/后续
  producer debugger 和完整 leak Gate仍待完成。
- A10 已在 A9 sealed channel 上关闭 bounded worker-attempt Secret recovery：过期 `starting|running` lease 可由更高 attempt
  reclaim；Control Plane 只接受 exact active positive-attempt lease，Backend 每个 execution 保持唯一 current resolution row，
  仅在 function/invocation identity不变时原子清空旧 ciphertext并替换 recipient。旧/同 attempt completion/replay、key drift、
  function/invocation drift均 fail closed；reclaimed running 不重复 state transition，reclaimed cancelling 在 snapshot/Secret/
  sandbox 前终止。该纵切只覆盖 read-only isolated code-export，不扩大 mutation、其他 permission 或跨 replica artifact/quota recovery。
- A11 已关闭 Backend Environment Secret at-rest KMS envelope/key rotation first vertical：每条 Secret 使用独立 data key，
  active versioned static key-ring只重包 data key；1-256 bounded `SKIP LOCKED` batch、单事务 update + aggregate-only audit、
  legacy direct-cipher migration 与旧 key retirement fence均已通过本地及 GitHub 真实 PostgreSQL 并发 Gate。at-rest provider/key id不进入
  Workspace、request、Control Plane、Worker envelope或 durable output。managed-cloud KMS adapter与完整跨表面 leak Gate继续关闭。
- ADR 47 已冻结 Binary Asset 的 transport-neutral owner、reference-only Workspace 保存态、Workspace-scoped digest/blob adapter、verified materialization、transform identity 与 delivery stop rules。`@prodivix/assets`、TypeScript/Go exact validator、PostgreSQL owner-only upload/read、commit-time reference fence、Web Resources upload/on-demand object URL、Blueprint Run/Workspace Test/ZIP Export composition、Compiler binary Export/Executable file、Remote bytes codec 和 living Golden PNG contract matrix 已形成 B0-B4 first vertical；B4 local-only 纵切现以独立 IndexedDB Workspace/digest blob partition 支持 exact upload/read、Resources preview/download、Run/Test/Export materialization 与 duplicate-before-reference/delete-after-author-state lifecycle，不把 bytes 写入 local project/Workspace wire。local -> cloud sync 已在发送前 materialize exact references，以 4 MiB manifest、256 unique blob、32 MiB 单对象和 128 MiB 总 bytes 的 bounded multipart 传输 raw parts；Backend 严格验证 headers/digest/length/media 与 missing/unreferenced/duplicate 集合，并在同一 PostgreSQL transaction 中提交 Project、Workspace、blob、documents，旧 JSON-only Asset import 继续 `AST-2004` fail closed。B5 已新增 deterministic PNG chunk/CRC/dimension 与 baseline JPEG segment/table/scan/orientation sanitizer、strict transformer/scanner-chain/derived-cache pipeline、各自 structural quarantine、policy-version cache re-scan、有界 ClamAV `INSTREAM` malware adapter、transport-neutral replica failover、required multi-engine chain、bounded fleet readiness/signature freshness、atomic fresh-update generation、old-session revocation/in-flight signing fence、Backend exact transform/media owner gateway 与独立 hash-capability Asset Delivery Host，active content inline hard cut，Web Resources 可生成 isolated sanitized PNG/baseline-JPEG preview 或受控 attachment。snapshot 不携带 blob locator/reference；unsupported JPEG、engine/replica exhaustion、daemon error/timeout/protocol/staleness/policy downgrade/divergence 不读取 upload 或创建 capability session。B6 已完成 Workspace-locked PostgreSQL retention/reference sweep、deterministic Git binary/LFS manifest/pointer/object projection、managed `.gitattributes`/stale-path lifecycle，以及 upload-receipt-fenced runtime filesystem Asset import/replace 单事务采纳。GitHub-only rootless real-daemon malware workflow 已取得首次远端通过证据；Browser JPEG 已由独立 Playwright Gate 覆盖 bytes-first upload、durable reference commit、reload exact-byte materialization、sanitize request 与 capability-origin decode，并进入 GitHub Smoke；第二 malware vendor、完整 raster re-encode/更多格式、public-CDN delivery，以及 Remote/Test/Build/Export 与第二 target 的真实 product image closure 仍待完成。
- provider-neutral snapshot、Remote codec/client/provider projection/Preview Bundle/Build Bundle/Test Report result、授权 artifact resolver、有界 HTTP transport、Backend auth gateway/durable execution grant、Control Plane/PostgreSQL/HTTP、Worker Agent，以及 D2 lease-fenced durable event/log、content-addressed artifact blob、总预算与 retention sweep 已完成；生产默认 rootless Podman sandbox adapter 和 remote-only GitHub Isolation Gate 已实现。Remote Preview 已通过 hash-only 短期 capability 子域、严格 CSP/Permissions Policy 与多文件 SPA fallback 接入独立 Host，Blueprint Run Mode 可显式选择 Browser/Remote；同一 living Golden Workspace 生成的 neutral snapshot 已覆盖 Browser Preview/Test 与 Remote Preview/Test/Build contract matrix，并经 strict Remote wire 进入真实 rootless install/Preview/Test/Build。dependency install 仅接入 internal network 的 hostname/443 allowlist proxy；Worker 启动前校验 exact policy，安装后断网并 inspect 确认为空才放行项目命令。Browser fetch/Data HTTP 运行请求和 install proxy 共同投影为 metadata-only `network.request`；Data correlation 固定 document/operation/invocation/sequence/attempt/source trace，Execution Center 只显示 strict decoded origin/timing/outcome/size。generated Remote Preview server/edge query/mutation 已通过父窗口 gateway 执行，iframe 只读取自身 capability runtime asset；mutation durable fence 阻止 uncertain replay，但不宣称 upstream distributed exactly-once。Remote Data trace 与 application Console 已通过 exact active-job Session observation 关联产品视图，React/Vite compile Gate 已拒绝无 gateway 的 server/edge Data；Remote 当前可持久化输出 canary 已双边 fail closed，Structured Console 已通过双预算投影与多层 copy redaction Gate；Remote Preview Terminal 已通过独立 wire/token、Backend gateway、Control Plane broker、Worker PTY、cursor reconnect、跨 chunk redaction 与 cleanup Gate，第二 target 继续建设。
- 真实 rootless Remote Golden CRUD parity、Structured Console/Files 完整 SourceTrace 调试旅程与单一第二 target 可移植性证明继续作为 G2 后续交付；广泛 Multi-target/Target SDK 生态属于 G6。当前 rootless runtime FS 已通过 capture-ready/PTTY-close handshake 形成 strict bounded diff artifact；Worker 对照 exact snapshot 重建 baseline/SourceTrace，Execution Center Files 允许用户显式采纳 revision/lifecycle-fenced whole-file CodeArtifact add/modify/delete，以及 exact document/baseline/media + upload-receipt-fenced Asset import/replace，并按确定性顺序形成单个可逆 Workspace Transaction 进入 Outbox/Atomic Commit；Asset delete 继续只由 Resources 显式流程拥有。
- Binary Asset 已有 ADR 47、B0-B5 local/cloud blob、upload-aware import、PNG/baseline-JPEG/ClamAV isolated + multi-engine/failover/fresh-update first vertical，以及 B6 Workspace-locked PostgreSQL retention/reference sweep、Git/LFS 与 runtime filesystem Asset import/replace；Auth/session/permission/server function 已有 ADR 46 及多条纵切，受限 execution-state live mutation、audited Remote live HMAC Secret、bounded canonical import graph、`workspace.owner` isolated permission、A9 dedicated isolated sealed Secret resolution、A10 worker-attempt recovery、A11 Backend KMS envelope/key rotation，以及 A12 Secret-free `workspace.read` isolated permission本地 Gate 已关闭；更新后的 rootless Gate 已配置等待本次远端证据。Binary Asset real-daemon remote Gate 已首次通过，仍需关闭 multi-vendor/full-raster/more-format/public delivery/cross-target product journey；Auth 主线仍需关闭 managed-cloud KMS adapter、`workspace.write`/其他 permission、项目源码 mutation isolated target、跨 replica artifact/quota recovery 与完整安全 Gate，二者完成后才能进入 G2 closure。
