...
| B5 transform、scanner 与 delivery                             | Implemented                  | deterministic PNG/JPEG sanitizer + Sharp full-raster re-encode、structural + required ClamAV/YARA-X chain、quarantine、replica/freshness/policy generation、bounded cache、capability Asset Delivery Host，以及双引擎 rootless real Gate evidence。          | 更多格式非 G2 blocker。                                        |
...
| B7 Golden 与 Browser product journey                          | Implemented                  | Living Golden blob-backed PNG、Browser JPEG upload/durable reload/full-raster/capability-origin decode、React/Vue exact-byte Browser/Test/Remote Preview/Test/Build matrix、protected static fail-close、Vue Chrome product Gate 与 GitHub matrix evidence。 | current G2 无剩余项。                                          |
