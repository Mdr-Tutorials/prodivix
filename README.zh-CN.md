...
Prodivix 仍处于快速演进的 alpha 阶段。当前产品门槛是 **G0 Passed / G1 Foundation**：Truth & Change Kernel 已形成可重复验证的闭环，语义化视觉/代码混合作者环境仍在建设。
...
| 领域                                  | 状态                                                                                                                                   |
| ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| 总体产品门槛                          | **G0 Passed / G1 Foundation**                                                                                                          |
| Truth & Change Kernel                 | G0 已通过：Canonical truth、History、Atomic Commit、revision conflict、Durable Outbox、local replica 与生产写入 Hard Cut 均已落地      |
| Diagnostics 与 Issues                 | G0 已通过：revision-aware 聚合、稳定 target、source span、Quick Fix 边界和编辑器回跳均已有覆盖                                         |
| Golden conformance 与 React/Vite 导出 | G0 非浏览器闭环已通过；独立 install、typecheck、test、browser smoke 与更完整 runtime 验证属于后续 Gate                                 |
| 语义化视觉/代码混合作者态             | G1 Foundation：已有 `CodeArtifact`、`CodeReference`、Code Slot 和 authoring registry；真实 Language Service 与受控 round-trip 仍在推进 |
| Blueprint、Route 与 PIR 作者态        | 基于 canonical Workspace 契约持续开发                                                                                                  |
| NodeGraph 与 Animation                | 已从 Web Hard Cut 为独立领域/runtime 内核；lifecycle、composition 与端到端行为验证仍属于后续工作                                       |
...
`Intent` 只能作为本地或 AI planner 的输入，不能成为生产 transport、Outbox entry kind 或 Atomic Commit wire operation。
...
- 在已配置的包内，优先使用 `@/...` 这类包内别名。
- code-owned 能力通过 Code Authoring Environment 与 Authoring Symbol Environment 接入；编辑器不持久化任意代码字符串，也不扫描其他编辑器的私有状态。
...
项目仍处于 alpha 阶段，会主动执行彻底的 Hard Cut。较大改动前，请先阅读相关产品阶段、架构决策和实施计划；除非当前契约明确要求，否则不要增加兼容层。
