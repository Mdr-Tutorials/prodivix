...
Prodivix 仍处于快速演进的 alpha 阶段。当前阶段状态只在 [`specs/roadmap/current-status.md`](specs/roadmap/current-status.md) 维护，README 不复制可能漂移的状态判断。
...
│   ├── code-language/        # revision-bound 语言能力与 Shader 编译能力
...
│   ├── i18n/                 # 国际化资源
│   ├── shared/               # 仍需跨领域共享的基础原语
│   ├── tokens/               # DTCG Format/Resolver current model、codec 与 Semantic Provider
│   ├── themes/               # 产品主题清单与 CSS 变量投影
│   ├── ui/                   # 共享 UI 组件
...
当前阶段状态的唯一来源是 [`specs/roadmap/current-status.md`](specs/roadmap/current-status.md)。Global Phase 定义与退出 Gate 位于 [`specs/roadmap/global-phases.md`](specs/roadmap/global-phases.md)，G0/G1 可重复证据位于相邻 evidence 文档。本 README 不再复制状态表。
...
| `pnpm run verify:g1:standalone` | 安装、类型检查、测试并构建 G1 导出项目            |
| `pnpm run verify:g1:browser`    | 运行 G1 导出项目并验证 WebGL2/WebGPU              |
...
`pnpm run verify:g0` 验证非浏览器 Truth & Change Kernel；`pnpm run verify:g1:standalone` 独立验证生成项目的安装、类型检查、测试与生产构建。`pnpm run verify:g1:browser` 会重复独立生产构建，通过临时本地 origin 验证路由、表单行为，并使用真实 WebGL2 与 WebGPU device 编译最小 shader。视觉回归、无障碍、性能与后续阶段正式 `VerificationEvidence` 仍由独立 Gate 验证。
...
| [`specs/roadmap/g1-closure-evidence.md`](specs/roadmap/g1-closure-evidence.md)                                                           | G1 Passed 的可重复验证证据                        |
...
| [`specs/implementation/g1-semantic-component-collection.md`](specs/implementation/g1-semantic-component-collection.md)                   | 已完成的 G1 语义/组件/Collection 实施计划         |
