...
func TestPutSnapshotPersistsOnlyEncryptedSecretMaterial(t *testing.T) {
	...
	mock.ExpectExec("INSERT INTO execution_environment_secret_materials").WithArgs("environment-1", sqlmock.AnyArg(), "access-token", sqlmock.AnyArg(), encryptedCanaryArgument{canary: []byte(canary)}).WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectCommit()
...
func TestUseSecretRequiresExactPrincipalSessionAndClearsCallbackMaterial(t *testing.T) {
	...
	mock.ExpectQuery(query).WithArgs("grant-1", "workspace-1", "environment-1", "revision-1", "principal-1", "session-1", "provider-1", "data-operation", "data/list", "access-token").WillReturnRows(sqlmock.NewRows([]string{"secret_bindings_json", "expires_at", "nonce", "ciphertext"}).AddRow(bindings, store.now().Add(time.Minute), nonce, ciphertext))
