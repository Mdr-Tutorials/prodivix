...
环境变量示例可以作为项目文件，真实秘密不能写入可导出的普通文档。`EnvironmentBindingReference` 与 `SecretRef` 已提供 reference-only identity，但 Secret store、resolver、runtime-zone permission 与 value provisioning 尚未交付；引用本身不代表秘密已经可用。
